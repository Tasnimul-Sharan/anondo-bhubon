...
export default function Navbar() {
  ...
  /*
   * Smooth scroll detection with separate show/hide thresholds.
   * This prevents flickering near the scroll boundary.
   */
  useEffect(() => {
    ...
    const updateNavbar = () => {
      ...
      setIsScrolled((previousState) => {
        if (!previousState && currentScrollY > 80) {
          return true;
        }

        if (previousState && currentScrollY < 20) {
          return false;
        }
  ...
  /*
   * Lock body scroll while the mobile navigation is open.
   */
  useEffect(() => {
  ...
  return (
    <>
      <header className="sticky left-0 top-0 z-[60] w-full">
        {/* Desktop Top Bar */}
        <div
          aria-hidden={isScrolled}
          className={`relative bg-[#202D72] text-white hidden transform-gpu transition-[grid-template-rows,opacity,transform] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] lg:grid ${
            isScrolled
        ...
        >
          <div className="overflow-hidden">
            <div className=" custom-container mx-auto">
              ...
              <div className="custom-container mx-auto flex h-[62px] items-center justify-between gap-8">
                {/* Contact Information */}
        ...
        {/* Main Navigation */}
        <div
          className={`border-b border-[#2C3A83]/10 bg-white backdrop-blur-xl transition-[box-shadow,background-color] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${
            isScrolled
              ? "shadow-[0_14px_40px_rgba(28,38,86,0.12)]"
              : "shadow-none"
          }`}
        >
          <div
            className={`custom-container mx-auto flex items-center justify-between transition-[height] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${
              isScrolled ? "h-[80px]" : "h-[102px]"
            }`}
          >
            {/* Logo */}
            <Link
              ...
              className={`relative z-10 flex shrink-0 items-center justify-center overflow-hidden transition-[width,height] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${
                isScrolled
                  ? "h-[68px] w-[185px]"
                  : "h-[96px] w-[225px]"
              }`}
            >
              <Image
                ...
                priority
                sizes="225px"
                className={`max-w-none object-contain transition-[height,transform] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${
                  isScrolled
                    ? "h-[92px] w-auto scale-[1.06]"
                    : "h-[128px] w-auto scale-[1.1]"
                }`}
              />
            </Link>
